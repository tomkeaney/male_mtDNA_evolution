...
# helper for saving large model objects and naming the file object.rds
...
# **THIS IS LIKELY TO BE CUT IF WE CAN DERIVE MARGINAL MEANS FROM MODELS**
...
**WHAT IS GOING ON WITH PREDICT?**

```{r female figure, fig.width=11, fig.height=8.5}

# Plotting with model predictions

# Make a new dataframe, for which we will derive predictions. It's the same as the old data, except that we set Focal haplotype, block and duplicate to the same value for all observations. Predictions change depending on random effect levels - this isn't what I want.
...
pred <- predict(female_avgm, se.fit = TRUE, type = "conditional", new_data) %>%
  unlist() %>% 
  as.data.frame()
...
pred_ZI <- predict(female_avgm, se.fit = TRUE, type = "zprob", new_data) %>%
  unlist() %>% 
  as.data.frame()
...
pred_social_ZI <- predict(female_avgm, se.fit = TRUE, type = "zprob", new_data_social) %>%
  unlist() %>% 
  as.data.frame()
...
**Figure 1:** mtDNA directly and indirectly affects female fitness. Panels **a** and **b** show the mean proportion of females that produced offspring (the zero-inflated or hurdle component of the model) across **a** female focal haplotypes and **b** social male haplotypes. Error bars depict standard errors. Panel **c** shows the direct effect of mtDNA on the number of offspring produced by a female. Black points show the mean with standard error for each haplotype, while coloured points represent each individual data point.


### Male adult fitness
* * *

We measure male fitness as the proportion of offspring sired by an experimental mitochondrial strain male while competing against a standard _bw_ male.
...
top_male_models <- get.models(male_binary_dredge, subset = delta < 6)
...
# Ok so this isn't very useful, the probabilities returned very a heap depending on the level set for the Individual random effect, and I can't get SE for some reason.

# Focal new data

new_data_male <- Male_fitness %>%
  ungroup() %>%
  select(Focal_haplotype, Strain, Block, Individual) %>%
  mutate(Social_haplotype = "Barcelona", Strain = "Barcelona 1", Block = "1", Individual = "32") %>% 
  distinct() %>% 
  slice(-2)


pred_male_focal <- predict(male_avgm, newdata = new_data_male, type = "response", se.fit = TRUE) %>%
  unlist() %>% 
  as.data.frame()
...
pred_focal_male <- cbind(new_data_male, pred_male_focal_1)
...
Male_focal_plot <- Male_fitness %>%
  ...
  scale_fill_manual(values = c("Barcelona" = "#fcde9c", "Brownsville" = "#f58670", "Dahomey" = "#e34f6f", "Israel" = "#d72d7c" , "Sweden" = "#7c1d6f")) +
  geom_point(data = pred_focal_male, aes(x = Focal_haplotype, y = mean_estimate), size = 3, colour='black') +
...
new_data_social_male <- Male_fitness %>%
  ungroup() %>%
  select(Social_haplotype, Strain, Block, Individual) %>%
  mutate(Focal_haplotype = "Barcelona", Strain = "Barcelona 1", Block = "1", Individual = "32") %>% 
  distinct()


pred_male_social <- predict(male_avgm, newdata = new_data_social_male, type = "response", se.fit = TRUE) %>%
  unlist() %>% 
  as.data.frame()
...
pred_male_social <- cbind(new_data_social_male, pred_male_social_1)
...
Male_social_plot <- Male_fitness %>%
  ...
  scale_size_continuous(range = c(0.5, 6), labels = NULL, breaks = c(20, 40, 60, 80, 100, 120)) +
 scale_fill_manual(values = c("Barcelona" = "#fcde9c", "Brownsville" = "#f58670", "Dahomey" = "#e34f6f", "Israel" = "#d72d7c" , "Sweden" = "#7c1d6f")) +
  geom_point(data = pred_male_social, aes(x = Social_haplotype, y = mean_estimate), size = 3, colour='black') +
  #geom_errorbar(data = pred_focal_male, aes(x = Focal_haplotype, ymax = Upper, ymin = Lower, width = 0), colour = "black") +
...
**Figure 2**: The proportion of offspring produced by _mt_-strain males competing with standard _bw_ males. **a** shows the direct effect of mtDNA on male fitness. **b** shows the indirect genetic effect of mtDNA on male fitness. Coloured points represent individual males, with larger points indicating a higher number of offspring produced in the vial (sired by either male). Black points show the mean proportion of offspring sired by the mt-strain male, with associated standard errors. **a** does not include a point estimate for the porportion of offspring sired by males carrying the Brownsville haplotype, as Brownsville was not included in the model. 
