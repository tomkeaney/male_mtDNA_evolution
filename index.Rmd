...
# Read in the bw male wing data

bw_male_wing_data <- read.csv("bw_male_wing_data.csv")
...
# Now lets remove vials where the female produced 0 offspring (this includes trials where the male died in development), as we cannot determine paternity from these vials
...
Duplicate strain: Each haplotype has been introgressed alongside the _w^1118^_ nuclear background in two independent duplicates, creating 10 total strains. Within each block we ran multiple replicates that were split in two: half used only duplicate one, while the other half used only duplicate two. This random effect accounts for any residual differences in their nuclear genome, or differences in the epigenome or microbiome that may have arisen between duplicates.
...
Dyad ID: accounts for differences in the quality of the environment that pairs of larvae experienced. For example, the moisture content of the food varied between pipette tips, despite our best efforts to keep this variable constant.  
...
if(file.exists("survival_dredge_strain.rds")){ # If already done, just load the results
  survival_dredge_strain <- readRDS("survival_dredge_strain.rds")
} else {survival_dredge_strain <- dredge(survival_model)                  # If not already done, run all the models and save the results
lapply(c("survival_dredge_strain"), save_it)
}


survival_table <- subset(survival_dredge_strain, delta < 6, recalc.weights = FALSE) %>% as.data.frame()
...
**Table 1**: Conditional model coefficients, standard error and 95% confidence limits are shown for the survivorship to adulthood averaged model. Bold rows indicate significant effects.
```{r survival model averaging}

# first get the RVI for each predictor

RVI_survival <- MuMIn::sw(survival_dredge_strain)

# average the models with delta < 6

survival_CIs <- confint(model.avg(survival_dredge_strain, subset = delta < 6)) %>% as.data.frame()

survival_estimate <- coefTable(model.avg(survival_dredge_strain, subset = delta < 6)) %>% as.data.frame()
...
```{r survival figure, fig.height=7, fig.width=10}

# focal haplotype survival plot
...
focal_survival_plot <- survival_mtDNA_summary %>%
  ...
  geom_point(data = survival_mtDNA_summary, aes(x = Focal_haplotype, y = Survived), size = 4, pch =21, colour='black', fill = c("Barcelona" = "#a50f15", "Brownsville" = "#fe9929", "Dahomey" = "#41b6c4", "Israel" = "#238443" , "Sweden" = "#4a1486")) +
  labs(x = "Focal mtDNA haplotype", y = "Proportion of larvae surviving to adulthood") +
  scale_y_continuous(limits = c(0.35, 0.525), breaks = c(0.35, 0.40, 0.45, 0.5)) +
  theme_minimal() +
  theme(legend.position = "none") +
  theme(panel.grid.major.x = element_blank())

# social haplotype survival plot

social_survival_mtDNA_summary <- survival %>% 
  dplyr::group_by(Social_haplotype) %>%
  dplyr::summarise(number_surviving = sum(Survived), number_died = sum(Survived == 0)) %>%
  as.data.frame()

social_survival_mtDNA_CIs <- get_CIs_for_binomial_trials(social_survival_mtDNA_summary$number_surviving, social_survival_mtDNA_summary$number_died) %>% rename(Survived = Proportion)

social_survival_mtDNA_summary <- cbind(social_survival_mtDNA_summary, social_survival_mtDNA_CIs)

social_survival_plot <- social_survival_mtDNA_summary %>%
  ggplot(aes(x = Social_haplotype, y = Survived, fill = Social_haplotype, colour = Social_haplotype)) +
  geom_errorbar(data = social_survival_mtDNA_summary, aes(x = Social_haplotype, ymax = upperCI, ymin = lowerCI, width = 0), colour = "black") +
  geom_point(data = social_survival_mtDNA_summary, aes(x = Social_haplotype, y = Survived), size = 4, pch =21, colour='black', fill = c("Barcelona" = "#a50f15", "Brownsville" = "#fe9929", "Dahomey" = "#41b6c4", "Israel" = "#238443" , "Sweden" = "#4a1486")) +
  labs(x = "Social mtDNA haplotype", y = "") +
  scale_y_continuous(limits = c(0.35, 0.525), breaks = c(0.35, 0.40, 0.45, 0.5)) +
  theme_minimal() +
  theme(legend.position = "none") +
  theme(panel.grid.major.x = element_blank())

ggarrange(focal_survival_plot, social_survival_plot, labels = c("a", "b"))
```

**Figure S2:** The points show the mean proportion of larvae surviving to eclose for each **a** focal haplotype and **b** social haplotype, and their associated 95% confidence limits.
...
**Figure S3**: The distribution of egg-to-adult development time, split by sex. Dashed lines indicate when lights were turned each morning and highlight the relationship between light and eclosion.
...
**Table 2:** Conditional model coefficients, standard error and 95% confidence limits are shown for the egg-to-adult development time averaged model. Bold rows indicate significant effects.
...
larval_development_plot_data %>%
  ...
  geom_quasirandom(data = larval_development_plot_data, width = 0.3, size = 2, alpha =  0.3) +
...
**Figure S4:** There are no large effects of focal mitochondrial haplotype on Egg-to-adult development time. The coloured points show the development time for individual flies. The black points show the mean development time for males and females and its 95% confidence limits. Dashed lines illustrate the point at which lights were turned on each morning.
...
**Table 3:** Model coefficients, standard error and 95% confidence limits are shown for the wing length top model. Bold rows indicate significant effects.
```{r size best model}
...
**Table S4:** Wing length (mm) for males and females pooled across the experimental _mt_-strains. We also measured the wings of a subset of the _bw_ males used in the male adult fitness assay. The size discrepancy between the _bw_ flies raised in vials at a moderate density (100 larvae per vial) and the _mt_-strains raised in highly competitive, resource poor, environments is evident.   

```{r size plot and table}
...
bw_wing_summary <- bw_male_wing_data %>%
  dplyr::summarise(Wing_length = mean(Length), Lower = (Wing_length - SE(Length)* 1.96), Upper = (Wing_length + SE(Length)*1.96), n = n())

bw_wing_summary$Sex <- "bw_male"
bw_wing_summary <- bw_wing_summary[, c(5, 1, 2, 3, 4)]


rbind(body_size_summary, bw_wing_summary) %>% pander(split.cell = 30, split.table = Inf, col.names = c("Sex", "Mean wing length (mm)", "Lower 95% CI", "Upper 95% CI", "n"))


size_plot <- body_size_plot_data %>%
...
#**Figure 5:** Sex affects wing length. The coloured points show the wing length for individual _mt_-strain flies. The black points show the mean wing length for males and females and its 95% confidence limits.
...
female_hurdle_model <- glmmTMB(Maternal_total_offspring ~ Focal_haplotype * Social_haplotype + (1|Strain) + (1|Block), data = female_reproductive_output, family = list(family="truncated_nbinom1",link="log"), ziformula = ~., na.action = na.fail, REML = FALSE)

```

#### Model evaluation

**Table S5**: Evaluation of the female reproductive output model. All possible models were evaluated from the global model that included an interaction between Focal haplotype and Social haplotype and the random factors Strain and Block. As there was no clear top model, the final model was calculated via model averaging. The zero-inflated results correspond to question (1), while the conditional results correspond to question (2) above.
```{r female dredge table}
# Compare all possible combinations of models (from the global model)

if(file.exists("female_dredge_strain.rds")){ # If already done, just load the results
  female_dredge_strain <- readRDS("female_dredge_strain.rds")
} else {female_dredge_strain <- dredge(female_hurdle_model)                  # If not already done, run all the models and save the results
lapply(c("female_dredge_strain"), save_it)
}

female_table <- subset(female_dredge_strain, delta < 6, recalc.weights = FALSE) %>% as.data.frame()
...
**Table 4:** Conditional (after hurdle) and zi (zero-hurdle requirement) model coefficients, standard error and 95% confidence limits are shown for the female offspring production averaged model. Bold rows indicate significant effects. 

```{r female model averaging}

# first get the RVI for each predictor

RVI_female <- MuMIn::sw(female_dredge_strain)

# summary(model.avg(female_dredge, subset = delta < 6))

Female_CIs <- confint(model.avg(female_dredge_strain, subset = delta < 6)) %>% as.data.frame()

Female_estimate <- coefTable(model.avg(female_dredge_strain, subset = delta < 6)) %>% as.data.frame()
...
female_social_zi_plot <- female_zi_plot_data %>%
  ...
  geom_errorbar(data = female_social_zi_summary, aes(x = Social_haplotype, ymax = upperCI, ymin = lowerCI, width = 0), colour = "grey26") +
  geom_point(data = female_social_zi_summary, aes(x = Social_haplotype, y = Binary_maternal_offspring), size = 4, pch =21, colour='grey26', fill = c("Barcelona" = "#a50f15", "Brownsville" = "#fe9929", "Dahomey" = "#41b6c4", "Israel" = "#238443" , "Sweden" = "#4a1486")) +
...
female_focal_zi_plot <- female_zi_plot_data %>%
  ...
  geom_point(data = female_focal_zi_summary, aes(x = Focal_haplotype, y = Binary_maternal_offspring), size = 4, pch =21, colour='grey26', fill = c("Barcelona" = "#a50f15", "Brownsville" = "#fe9929", "Dahomey" = "#41b6c4", "Israel" = "#238443" , "Sweden" = "#4a1486")) +
...
ggarrange(female_focal_zi_plot, female_focal_cond_plot, female_social_zi_plot, female_social_cond_plot, labels = c("a", "b", "c", "d"))

```

**Figure 1:** mtDNA affects the direct and inclusive components of fitness. Panels **a** and **b** show the direct effect of mtDNA on female fitness, for the hurdle (did the female produce any offspring?) and conditional (how many offspring did the female produce?) components of the model respectively. Panels **c** and **d** show the social effect of mtDNA on female fitness. **a** and **c** Points show the mean for each focal/social haplotype and associated confidence intervals. **b** and **d** Black points show the mean for each haplotype, while coloured points represent each individual data point.


### Male adult fitness

We follow the classic approach to sperm competition analysis and consider male fitness as the proportion of offspring sired by an experimental mitochondrial strain male while competing against a standard _bw_ male.
...
The Brownsville haplotype renders males sterile alongside the _w^1118^_ nuclear background and sub-fertile alongside all other tested backgrounds. In our experiment, we find that Brownsville males are able to produce offspring but to a very limited capacity. Due to this, our model will not converge with the Brownsville males in the data-set. Therefore, we acknowledge that there is an effect of the Brownsville focal haplotype on male fitness and then remove it from the data-set.
...
response <- cbind(Male_fitness_without_Brownsville$Focal_male_offspring, Male_fitness_without_Brownsville$bw_male_offspring)

Binary_male_model <- lme4::glmer(response ~ Focal_haplotype * Social_haplotype + (1|Strain) + (1|Block) + (1|Individual), data = Male_fitness_without_Brownsville, family = "binomial", control = glmerControl(optimizer = "Nelder_Mead", optCtrl=list(maxfun=100000)), na.action = na.fail)
...
**Table 5:** Model coefficients, standard error and 95% confidence limits are shown for the male adult fitness averaged model. Bold rows indicate significant effects. 

```{r male model averaging}

# first get the RVI for each predictor

RVI_male <- MuMIn::sw(male_binary_dredge)
...
row.names(Male_binary_model_avg) <- c("Intercept", "Social haplotype: Brownsville", "Social haplotype: Dahomey", "Social haplotype: Israel", "Social haplotype: Sweden", "Focal haplotype: Dahomey", "Focal haplotype: Israel", "Focal haplotype: Sweden")
...
Male_fitness_summary_socialmt <- Male_fitness %>% 
...
Male_fitness_summary_focalmt <- Male_fitness %>% 
...
Male_focal_plot <- Male_fitness %>%
  ...
  geom_quasirandom(data = Male_fitness, width = 0.3, alpha =  0.3, aes(size = Offspring_counted)) +
...
Male_social_plot <- Male_fitness %>%
  ...
  labs(x = "Social female mtDNA haplotype", y = "") +
  theme_minimal() +
  theme(legend.position = "none") +
  theme(panel.grid.major.x = element_blank())
...
**Figure 2**: The proportion of offspring produced by _mt_-strain males competing with standard _bw_ males. **a** shows the direct effect of mtDNA on male fitness. **b** shows the indirect genetic effect of mtDNA on male fitness. Coloured points represent individual males, with larger points indicating a higher number of offspring produced in the vial (sired by either male). Black points show the mean proportion of offspring sired by the mt-strain male, with associated 95% confidence intervals.
...
**Table S6**: the raw data-set used in the present study.
```{r raw data}
