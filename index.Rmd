...
pred <- predict(female_avgm, se.fit = TRUE, type = "conditional", re.form = NA, new_data) %>%
  unlist() %>% 
  as.data.frame()
...
pred_ZI <- predict(female_avgm, se.fit = TRUE, type = "zprob", re.form = NA, new_data) %>%
  unlist() %>% 
  as.data.frame()
...
pred_social_ZI <- predict(female_avgm, se.fit = TRUE, type = "zprob", re.form = NA, new_data_social) %>%
  unlist() %>% 
  as.data.frame()
...
Male_model_glmmTMB <- glmmTMB(response ~ Focal_haplotype * Social_haplotype + (1|Strain) + (1|Block) + (1|Individual), data = Male_fitness_without_Brownsville, family = "binomial", ziformula = ~0, na.action = na.fail)
...
} else {male_dredge <- dredge(Male_model_glmmTMB) # If not already done, run all the models and save the results
...
row.names(Male_model_avg) <- c("Intercept", "Social haplotype: Brownsville", "Social haplotype: Dahomey", "Social haplotype: Israel", "Social haplotype: Sweden", "Focal haplotype: Dahomey", "Focal haplotype: Israel", "Focal haplotype: Sweden")
...
new_data_male <- Male_fitness %>%
  ...
  mutate(Social_haplotype = "Barcelona", Strain = "Barcelona 1", Block = "1", Individual = "4") %>% 
  distinct() %>% 
  slice(-2)


pred_male_focal <- predict(male_avgm, newdata = new_data_male, type = "conditional", se.fit = TRUE, re.form = NA) %>%
  unlist() %>% 
  as.data.frame()
...
new_data_social_male <- Male_fitness %>%
  ...
  mutate(Focal_haplotype = "Barcelona", Strain = "Barcelona 1", Block = "1", Individual = "4") %>% 
  distinct()


pred_male_social <- predict(male_avgm, newdata = new_data_social_male, type = "conditional", se.fit = TRUE, re.form = NA) %>%
  unlist() %>% 
  as.data.frame()
