...
**(focal_male_offspring, Offspring_counted) ~ Focal_haplotype * Social_haplotype + Duplicate + (1|Individual) + (1|Block)**
...
names(Male_table)[names(Male_table) == "cond((Int))"] <- "Conditional intercept"
names(Male_table)[names(Male_table) == "zi((Int))"] <- "Zero-inflated intercept"
names(Male_table)[names(Male_table) == "disp((Int))"] <- "Dispersion factor intercept"
names(Male_table)[names(Male_table) == "cond(factor(duplicate))"] <- "Conditional (Duplicate)"
names(Male_table)[names(Male_table) == "cond(Focal_haplotype)"] <- "Conditional (Focal haplotype)"
names(Male_table)[names(Male_table) == "cond(Social_haplotype)"] <- "Conditional (Social haplotype)"
names(Male_table)[names(Male_table) == "cond(Focal_haplotype:Social_haplotype)"] <- "Conditional (Focal haplotype x Social haplotype)"
names(Male_table)[names(Male_table) == "zi(factor(duplicate))"] <- "Zero-inflated (Duplicate)"
names(Male_table)[names(Male_table) == "zi(Focal_haplotype)"] <- "Zero-inflated (Focal haplotype)"
names(Male_table)[names(Male_table) == "zi(Social_haplotype)"] <- "Zero-inflated (Social haplotype)"
names(Male_table)[names(Male_table) == "zi(Focal_haplotype:Social_haplotype)"] <- "Zero-inflated (Focal haplotype x Social haplotype)"
...
pander(Male_model_avg, split.cell = 40, split.table = Inf, emphasize.strong.rows = c(3, 6, 7, 9, 10, 12:15, 17, 19, 20))

```


```{r}
male_zi_plot_data <- Male_fitness %>%
  mutate(Binary_paternal_offspring = ifelse(Focal_male_offspring > 0, "1", "0"))

male_zi_summary <- male_zi_plot_data %>% 
  dplyr::group_by(Social_haplotype) %>%
...
male_zi_CIs <- get_CIs_for_binomial_trials(male_zi_summary$number_producing_offspring, male_zi_summary$number_zero_offspring) %>% rename(Binary_paternal_offspring = Proportion)

male_zi_summary <- cbind(male_zi_summary, male_zi_CIs)

male_zi_plot_data %>%
  ggplot(aes(x = Social_haplotype, y = Binary_paternal_offspring, fill = Social_haplotype, colour = Social_haplotype)) +
  geom_point(data = male_zi_summary, aes(x = Social_haplotype, y = Binary_paternal_offspring), size = 3, colour='black') +
  geom_errorbar(data = male_zi_summary, aes(x = Social_haplotype, ymax = upperCI, ymin = lowerCI, width = 0), colour = "black") +
...
**Figure 8:** The proportion of males successfully producing offspring is affected by the mtDNA of a social female competitor during larval development. Black points show the mean for each social haplotype and associated confidence intervals. Data corresponds to the zero-inflated (binomial) component of the hurdle model.
...
male_cond_summary <- male_cond_plot_data %>% 
  dplyr::group_by(Social_haplotype) %>%
...
Male_cond_plot <- male_cond_plot_data %>%
  ggplot(aes(x = Social_haplotype, y = Proportion_focal, fill = Social_haplotype, colour = Social_haplotype)) +
  geom_quasirandom(data = male_cond_plot_data, width = 0.3, alpha =  0.5, aes(size = Offspring_counted)) +
