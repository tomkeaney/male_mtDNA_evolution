...
library(glmmTMB) # for zero-inflated glms
...
fitness_data <- dplyr::select(all_data, Individual, Block, duplicate,  Replicate, Sex, Focal.haplotype, Social.haplotype, Mortality,  Social.Survival, Development.time..hrs., Day.emerged, Hours.from.lights.on, Wing.size..mm., Female.offspring, Male.offspring, Total.female.assay, Total.red.all.vials, Total.bw.all.vials) %>% 
...
Male_fitness <- select(Male_fitness, Individual, Block, Focal.haplotype, Social.haplotype, Social.Survival,  Mortality, Development.time..hrs., Day.emerged, Wing.size..mm., Hours.from.lights.on, Total.red.all.vials, Total.bw.all.vials, Offspring_counted,  Proportion.red.all.vials, duplicate) %>% 
...
_Focal haplotype:_ the mtDNA haplotype that an individual carries.

_Social haplotype:_ the mtDNA haplotype that a social partner during larval development carries.

_Social survival:_ the survival outcome of the social partner.

_Sex:_ is the focal individual female or male? The social partner will always be of the opposite sex to the focal individual.

We also include:

_Duplicate:_ Each haplotype has been introgressed alongside the _w^1118^_ nuclear background in two independent duplicates. WIthin each block we ran multiple replicates that were split in two: half used only duplicate one will the other half used only duplicate two. This fixed effect accounts for any nuclear differences that may have arisen between duplicates.

**Random effects**

_Block:_ accounts for differences in the response variable between experimental blocks (e.g. to variance in temperature or composition of the fly food). In our experiment a Block contained multiple replicates and a replicate was made up of 25 different cells each housing a pair of larvae.
...
**Survived ~ Focal_haplotype * Social_haplotype * Social_survival + Sex + Duplicate + (1|Block)**

```{r}

# Fit the global model

survival_model <- lme4::glmer(Survived ~ Focal_haplotype * Social_haplotype * Social_survival + Sex + factor(Duplicate) + (1|Block), data = survival, family = "binomial", control = glmerControl(optimizer = "Nelder_Mead", optCtrl=list(maxfun=100000)), na.action = na.fail)
...
names(survival_table)[names(survival_table) == "Social_survival"] <- "Social survival"
names(survival_table)[names(survival_table) == "Focal_haplotype:Social_survival"] <- "Focal haplotype x Social survival"
names(survival_table)[names(survival_table) == "Focal_haplotype:Social_haplotype"] <- "Focal haplotype x Social haplotype"
names(survival_table)[names(survival_table) == "Social_haplotype:Social_survival"] <- "Social haplotype x Social survival"
names(survival_table)[names(survival_table) == "Focal_haplotype:Social_haplotype:Social_survival"] <- "Focal haplotype x Social haplotype x Social survival"
...
pander(survival_model_avg, split.cell = 40, split.table = Inf, emphasize.strong.rows = (2:4))
...
```{r}
survival_mtDNA_summary <- survival_plot_data %>% 
...
**Figure 2:** The black points show the mean proportion of larvae surviving to eclose for each mtDNA haplotype and its 95% confidence limits.
...
**Dev_time ~ Focal_haplotype * Social_haplotype * Social_survival + Sex + Duplicate + (1|Block)**

```{r}

# Fit the linear model

linear_dev_model <- lmer(Dev_time ~ Focal_haplotype * Social_haplotype * Social_survival + Sex + factor(Duplicate) + (1|Block), larval_development, na.action = na.fail, REML = FALSE)
...
names(development_table)[names(development_table) == "Social_survival"] <- "Social survival"
names(development_table)[names(development_table) == "Social_haplotype"] <- "Social haplotype"
names(development_table)[names(development_table) == "Focal_haplotype:Social_survival"] <- "Focal haplotype x Social survival"
names(development_table)[names(development_table) == "Focal_haplotype:Social_haplotype"] <- "Focal haplotype x Social haplotype"
names(development_table)[names(development_table) == "Social_haplotype:Social_survival"] <- "Social haplotype x Social survival"
names(development_table)[names(development_table) == "Focal_haplotype:Social_haplotype:Social_survival"] <- "Focal haplotype x Social haplotype x Social survival"
...
pander(Dev_model_avg, split.cell = 40, split.table = Inf, emphasize.strong.rows = c(2:4, 7, 16))
...
**Figure 3:** Egg-to-adult development time is affected by an individual's sex and the success of a social competitor. The coloured points show the development time for individual flies. The black points show the mean development time for males and females and its 95% confidence limits.
...
larval_development_plot_data %>%
  ...
  labs(x = "Female mtDNA haplotype", y = "Egg-to-adult development time (hrs)") +
  theme_minimal() +
  theme(legend.position = "none") +
  theme(panel.grid.major.x = element_blank())
```

**Figure 4:** Egg-to-adult development time is affected by an individual's mtDNA haplotype. The coloured points show the development time for individual flies. The black points show the mean development time for males and females and its 95% confidence limits.
...
**Wing_length ~ Focal_haplotype * Social_haplotype * Social_survival + Sex + Duplicate + (1|Block)**


```{r}

body_size_model<- lmer(Wing_length ~ Focal_haplotype * Social_haplotype * Social_survival + Sex + factor(Duplicate) + (1|Block), body_size, na.action = na.fail, REML = FALSE)
...
names(size_table)[names(size_table) == "Social_survival"] <- "Social survival"
names(size_table)[names(size_table) == "Social_haplotype"] <- "Social haplotype"
names(size_table)[names(size_table) == "Focal_haplotype:Social_survival"] <- "Focal haplotype x Social survival"
names(size_table)[names(size_table) == "Focal_haplotype:Social_haplotype"] <- "Focal haplotype x Social haplotype"
names(size_table)[names(size_table) == "Social_haplotype:Social_survival"] <- "Social haplotype x Social survival"
names(size_table)[names(size_table) == "Focal_haplotype:Social_haplotype:Social_survival"] <- "Focal haplotype x Social haplotype x Social survival"
...
pander(Size_model_avg, split.cell = 40, split.table = Inf, emphasize.strong.rows = (2:4))
...
body_size_plot_data %>%
  ggplot(aes(x = Social_survival, y = Wing_length, fill = Social_survival, colour = Social_survival)) +
  geom_quasirandom(data = body_size_plot_data, width = 0.3, size = 2, alpha =  0.5) +
  scale_colour_manual(values = c("Died as larva" = "#a50f15", "Died as pupa" = "#fe9929", "Survived to adulthood" = "#41b6c4")) +
  geom_point(data = body_size_summary, aes(x = Social_survival, y = Wing_length), size = 3, colour='black') +
  geom_errorbar(data = body_size_summary, aes(x = Social_survival, ymax = Upper, ymin = Lower, width = 0), colour = "black") +
  facet_wrap(~ Sex) +
  labs(x = "Survival outcome of social partner", y = "Wing length (mm)") +
  theme_minimal() +
  theme(legend.position = "none") +
  theme(panel.grid.major.x = element_blank())
...
**Maternal_total_offspring ~ Focal_haplotype * Social_haplotype + Social_haplotype * Social_survival + factor(Duplicate) + (1|Block)**

```{r}

female_hurdle_model <- glmmTMB(Maternal_total_offspring ~ Focal_haplotype * Social_haplotype + Social_haplotype * Social_survival + factor(Duplicate) + (1|Block), data = female_reproductive_output, family = list(family="truncated_nbinom1",link="log"), ziformula = ~., na.action = na.fail, REML = FALSE)
...
names(female_table)[names(female_table) == "cond(Social_survival)"] <- "Conditional (Social survival)"
...
names(female_table)[names(female_table) == "cond(Social_haplotype:Social_survival)"] <- "Conditional (Social haplotype x Social survival)"
...
names(female_table)[names(female_table) == "zi(Social_survival)"] <- "Zero-inflated (Social survival)"
...
names(female_table)[names(female_table) == "zi(Social_haplotype:Social_survival)"] <- "Zero-inflated (Social haplotype x Social survival)"
...
**Table S5**: Anova results from zero-inflated binomial model for male reproductive fitness. 
```{r, eval=FALSE}
Male_fitness_without_Brownsville <- Male_fitness %>% filter(Focal_haplotype != "Brownsville") %>% mutate(Individual = 1:n())

Male_fitness_without_Brownsville$test <- sample(c(0,1), nrow(Male_fitness_without_Brownsville), replace = T)

male_zi_model <- glmmTMB(cbind(Focal_male_offspring, Bw_offspring) ~ Focal_haplotype * Social_haplotype + Social_haplotype * Social_survival + factor(Duplicate) + (1|Block) + (1|Individual), data = Male_fitness_without_Brownsville, family = binomial, ziformula = ~., na.action = na.fail, REML = FALSE)

dredge(male_zi_model)

Anova(male_zi_model) %>% pander(split.cell = 40, split.table = Inf)
