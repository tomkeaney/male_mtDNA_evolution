...
Male_fitness <- dplyr::select(Male_fitness, Individual, Block, Focal.haplotype, Social.haplotype, Social.Survival,  Mortality, Development.time..hrs., Day.emerged, Wing.size..mm., Hours.from.lights.on, Total.red.all.vials, Total.bw.all.vials, Offspring_counted,  Proportion.red.all.vials, duplicate) %>% 
...
# The data follows an unusual distribution; see plot below.

hist(Male_fitness_without_Brownsville$Proportion_focal)

# A zero-one inflated beta distribution is appropriate here; see https://www.theanalysisfactor.com/zero-one-inflated-beta-models-for-proportion-data/

# We can use gamlss or a bayesian method

# Male_fitness_without_Brownsville$test <- sample(c(0,1), nrow(Male_fitness_without_Brownsville), replace = T)

male_zi_model <- glmmTMB(cbind(Focal_male_offspring, Bw_offspring) ~ Focal_haplotype * Social_haplotype + Social_haplotype * Social_survival + factor(Duplicate) + (1|Block) + (1|Individual), data = Male_fitness_without_Brownsville, family = binomial, ziformula = ~., na.action = na.fail, REML = FALSE, control = glmmTMBControl(optCtrl = list(iter.max = 1000, eval.max = 1000), profile = TRUE, collect = FALSE))

dredge(male_zi_model)

# This returns the following error message:

# Error in as.symbol(opt$response) : 'pairlist' object cannot be coerced to type 'symbol'
